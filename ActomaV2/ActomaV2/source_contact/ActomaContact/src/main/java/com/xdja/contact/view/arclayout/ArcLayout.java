...
public class ArcLayout extends ViewGroup {
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    for (int i = 0, size = getChildCount(); i < size; i++) {
      ...
      childMeasureBy(child, point.x, point.y);
      childLayoutBy(child, point.x, point.y);
...
//    arcDrawable.draw(canvas);
  ...
  protected void childLayoutBy(View child, int x, int y) {
    ...
    switch (origin & ArcOrigin.HORIZONTAL_MASK) {
      case ArcOrigin.LEFT:
        Log.e("ArcLayout", "LEFT");
        left = x;
        break;
      case ArcOrigin.RIGHT:
         Log.e("ArcLayout", "RIGHT");
        left = x - width;
        break;
      default:
        Log.e("ArcLayout", "default");
        left = x - (width / 2);
    }
   ...
   * @param y
   */
  public void computeOrigin(int x,int y) {
    origin.set(x,y);
    computePosition();
    requestLayout();
  }
  ...
  private void computePosition(){
    ...
    final int radius = (axisRadius == DEFAULT_AXIS_RADIUS)
            ...
            : axisRadius;
    int num=getChildCountWithoutGone()-2;
    if (num==0)
      num=1;
    final float perDegrees = arc.computePerDegrees(num);
    int arcIndex = 0;
    for (int i = 1, size = getChildCount(); i < size; i++) {
...
//    requestLayout();
  ...
  public Point getPositionByIndex(int index){
    if (points.size()==0)
      return null;
