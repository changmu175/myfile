...
//      return new Point(r, centerY(t, b));
  ...
  public float computeDegrees(int index, float perDegrees) {
    final float offsetAngle =
            (sweepAngle < 360) ? startAngle - perDegrees  :
            startAngle;
    float degree=offsetAngle
            + perDegrees
            + (perDegrees * index);
//    float degree=startAngle+perDegrees*index;
    Log.e("Arc", "startAngle= " + startAngle + "  sweepAngle= " + sweepAngle + "  perDegrees= " + perDegrees + " offsetAngle=" + offsetAngle + "  degree= " + degree);
    return degree;
  }
