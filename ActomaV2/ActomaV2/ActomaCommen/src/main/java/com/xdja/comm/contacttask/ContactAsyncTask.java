 ...
 * <li>3. 支持子线程建立并执行{@link ContactAsyncTask}，{@link #onPostExecute(Object)}方法一定会在主线程执行</li>
 * </ul>
 * @author MaTianyu
 * 2014-1-30下午3:10:43
 */
public abstract class ContactAsyncTask<Params, Progress, Result> {
	private static final String LOG_TAG = "ContactAsyncTask";
	...
	private static final ThreadFactory sThreadFactory = new ThreadFactory() {
		...
		public Thread newThread(Runnable r) {
			return new Thread(r, "ContactAsyncTask #" + mCount.getAndIncrement());
		}
	};
	...
	private static class SmartSerialExecutor implements Executor {
		...
		//private ArrayDeque<Runnable> mQueue = new ArrayDeque<Runnable>(serialMaxCount);
	...
	public enum Status {
		...
		RUNNING,
		/**
		 * Indicates that {@link ContactAsyncTask#onPostExecute} has finished.
		 */
		FINISHED,
	}
	 ...
	 * <p> 加载类似瀑布流时产生的大量并发（一定程度允许任务被剔除队列）时请用{@link ContactAsyncTask#executeAllowingLoss(Object...)}
	 * @param params The parameters of the task.
	 *
	 * @return This instance of ContactAsyncTask.
	 *
	 ...
	 */
	public final ContactAsyncTask<Params, Progress, Result> execute(final Params... params) {
	 ...
	 * @return
	 */
	public final ContactAsyncTask<Params, Progress, Result> executeAllowingLoss(Params... params) {
	 ...
	 * ContactAsyncTask, however you can also use your own {@link Executor} for custom
	 * behavior.
	 *
	 * <p>This method must be invoked on the UI thread.
	 *
	 ...
	 * @see #execute(Object[])
	 */
	public final ContactAsyncTask<Params, Progress, Result> executeOnExecutor(Executor exec, Params... params) {
	 ...
	 * <p> 加载类似瀑布流时产生的大量并发（任务数超出限制允许任务被剔除队列）时请用{@link ContactAsyncTask#executeAllowingLoss(Runnable)}
	 * @see #execute(Object[])
	 * @see #executeOnExecutor(Executor, Object[])
	 */
	...
	private static class AsyncTaskResult<Data> {
		final ContactAsyncTask mTask;
		final Data[] mData;

		AsyncTaskResult(ContactAsyncTask task, Data... data) {
			mTask = task;
			mData = data;
		}
	}
