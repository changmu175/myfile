...
public class SipConfigManager {
	...
	/* BEGIN: options if client request missed calls 20141107 */
	public static final String REQUEST_MISSED_CALLS = "request_missed_calls";
	public static final String NEED_REQUEST_MISSED_CALLS = "need_request_missed_calls";
//    public static final String SIP_SERVER_PORT = "sip_server_port";
    ...
    //public static final String AUTHORITY = "com.actoma.csipsimple.prefs";
    public static String AUTHORITY = BuildConfigHelper.APPLICATION_ID + ".preferenceprovider";
    /** 20170222-mengbo-end **/
     ...
     * Deeply advised to not use directly
     */
    public static Uri basePreferenceIdURI = null;

    // Raz
    /**
     * Reset uri to wipe the entire preference database clean.
     */
    public static Uri razURI = null;
    ...
    public static String getCodecKey(String codecName, String type) {
        ...
        if (codecParts.length >= 2) {
            return "codec_" + codecParts[0].toLowerCase(Locale.getDefault()) + "_" + codecParts[1] + "_" + type;
        }
    ...
    private static Uri getBasePreferenceIdUri(Context context) {
        if(basePreferenceIdURI == null || basePreferenceIdURI.equals(Uri.EMPTY)){
            if(context != null){
                basePreferenceIdURI = Uri.parse(ContentResolver.SCHEME_CONTENT + "://"
                        + PreferenceProvider.getAuthority(context) + "/" + PREFS_TABLE_NAME + "/");
            }else{
                basePreferenceIdURI = Uri.EMPTY;
            }
        }
        return basePreferenceIdURI;
    }

    private static Uri getPrefUriForKey(Context context, String key) {
        //20170224-mengbo : 动态获取URI
        //return Uri.withAppendedPath(PREF_ID_URI_BASE, key);
        return Uri.withAppendedPath(getBasePreferenceIdUri(context), key);
    }

    public static Uri getRazURI(Context context) {
        if(razURI == null || razURI.equals(Uri.EMPTY)){
            if(context != null){
                razURI = Uri.parse(ContentResolver.SCHEME_CONTENT + "://"
                        + PreferenceProvider.getAuthority(context) + "/" + RESET_TABLE_NAME);
            }else{
                razURI = Uri.EMPTY;
            }
        }
    ...
    public static String getPreferenceStringValue(Context context, String key, String defaultValue) {
        ...
        Uri uri = getPrefUriForKey(context, key);
        Cursor c=null;
        /**Begin:sunyunlei 添加异常捕获，关闭 Cursor 20141229**/
        try{
        	c = context.getContentResolver().query(uri, null, String.class.getName(), null, null);
        	if (c != null) {
        		/**Begin:sunyunlei 添加cursor 数据为空时的判断 20141229**/
		        if(c.moveToFirst()){
		        /**End:sunyunlei 添加cursor 数据为空时的判断 20141229**/
        ...
        /**End:sunyunlei 添加异常捕获，关闭 Cursor 20141229**/
        return value;
    }
    ...
    public static Boolean getPreferenceBooleanValue(Context ctxt, String key) {
        return getPreferenceBooleanValue(ctxt, key, false);
    }
    ...
    public static Boolean getPreferenceBooleanValue(Context context, String key, Boolean defaultValue) {
        ...
        Cursor c=null;
        try{
		    c = context.getContentResolver().query(uri, null, Boolean.class.getName(), null, null);
		    if (c != null) {
		        /**Begin:sunyunlei 添加cursor 数据为空时的判断 20141229**/
		        if(c.moveToFirst()){
		        /**End:sunyunlei 添加cursor 数据为空时的判断 20141229**/
     ...
     */
    public static Float getPreferenceFloatValue(Context context, String key) {
        return getPreferenceFloatValue(context, key, null);
    }
     ...
     * @return the preference value
     */
    public static Float getPreferenceFloatValue(Context context, String key, Float defaultValue) {
        Float value = defaultValue;
        Uri uri = getPrefUriForKey(context, key);
        /**Begin:sunyunlei 添加异常捕获，关闭 Cursor 20141229**/
        Cursor c=null;
        try{
	        c = context.getContentResolver().query(uri, null, Float.class.getName(), null, null);
	        if (c != null) {
	        	/**Begin:sunyunlei 添加cursor 数据为空时的判断 20141229**/
	            if(c.moveToFirst()){
	            /**End:sunyunlei 添加cursor 数据为空时的判断 20141229**/
     ...
     */
    public static Integer getPreferenceIntegerValue(Context context, String key) {
        return getPreferenceIntegerValue(context, key, null);
    }
     ...
     * @return the preference value
     */
    public static Integer getPreferenceIntegerValue(Context context, String key, Integer defaultValue) {
        Integer value = defaultValue;
        Uri uri = getPrefUriForKey(context, key);
        /**Begin:sunyunlei 添加异常捕获，关闭 Cursor 20141229**/
        Cursor c=null;
        try{
	        c = context.getContentResolver().query(uri, null, Integer.class.getName(), null, null);
	        if (c != null) {
	        	/**Begin:sunyunlei 添加cursor 数据为空时的判断 20141229**/
	            if(c.moveToFirst()){
	            /**End:sunyunlei 添加cursor 数据为空时的判断 20141229**/
     ...
     */
    public static void setPreferenceStringValue(Context context, String key, String value) {
        Uri uri = getPrefUriForKey(context, key);
        ContentValues values = new ContentValues();
        values.put(SipConfigManager.FIELD_VALUE, value);
        context.getContentResolver().update(uri, values, String.class.getName(), null);
    }
     ...
     * @param value The value to set to
     */
    public static void setPreferenceBooleanValue(Context context, String key, boolean value) {
        Uri uri = getPrefUriForKey(context, key);
        ContentValues values = new ContentValues();
        values.put(SipConfigManager.FIELD_VALUE, value);
        context.getContentResolver().update(uri, values, Boolean.class.getName(), null);
    }
     ...
     * @param value The value to set to
     */
    public static void setPreferenceFloatValue(Context context, String key, Float value) {
        Uri uri = getPrefUriForKey(context, key);
        ContentValues values = new ContentValues();
        values.put(SipConfigManager.FIELD_VALUE, value);
        context.getContentResolver().update(uri, values, Float.class.getName(), null);
    }
     ...
     * @param value The value to set to
     */
    public static void setPreferenceIntegerValue(Context context, String key, Integer value) {
        if (value != null) {
            setPreferenceStringValue(context, key, value.toString());
        }
    }
