...
public final class PjSipCalls {
    ...
    public static void updateSessionFromPj(SipCallSessionImpl session, pjsip_event e, Context context)
            throws SameThreadException {
        LogUtil.getUtils(THIS_FILE).d("Update call " + session.getCallId());
        ...
        if (status == pjsua.PJ_SUCCESS) {
            ...
            if(e != null) {
                ...
                LogUtil.getUtils(THIS_FILE).d("Last status code is " + status_code);
            ...
            /* BEGIN: Flag indicate if this call is in hanging process. add by xjq, 2015/1/28 */
            //session.setIsHanging(pjInfo.getIs_hanging() == pjsuaConstants.PJ_TRUE);
            /* END: Flag indicate if this call is in hanging process. add by xjq, 2015/1/28 */
            
            // Set call info role.Add by xjq, 2015/3/4
            //session.setRole(pjInfo.getIs_uac()==1?0:1);

        } else {
            LogUtil.getUtils(THIS_FILE).d("Call info from does not exists in stack anymore - assume it has been disconnected");
    ...
    private static void updateSession(SipCallSessionImpl session, pjsua_call_info pjCallInfo,
        ...
        // 设置会话的call-id xjq 2015-12-22
        session.setCallIdString(PjSipService.pjStrToString(pjCallInfo.getCall_id()));
