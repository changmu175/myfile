...
public class SipProfile implements Parcelable {
    ...
    /**
     *自动选择通讯协议。
     *默认情况下它使用UDP，如果数据包大于UDP的限制，将切换
     *为TCP。
     *要注意这一点，因为不是所有的SIP服务器支持UDP / TCP
     */
     ...
     * @see #datatype
     * 普通密码模式
     * 参见  "http://www.pjsip.org/pjsip/docs/html/structpjsip__cred__info.htm#a8b1e563c814bdf8012f0bdf966d0ad9d"
     * pjsip 文档 中数据类型
     */
     ...
     */
    public int reg_timeout = 60*30;			//修改注册超时时间为半个小时,Modify by xjq, 2015/3/11
    /**
     ...
     */
    public int reg_delay_before_refresh = -1;// xjq 2015-09-02
    /**
     ...
     */
    public boolean use_rfc5626 = false; //rfc5626规定的一种的NAT穿越方法，密信无此需求. Modify by xjq, 2015/3/11
    /**
     ...
     */
    public boolean mwi_enabled = false;						//取消订阅消息发送-Mod-Lixin-0421
    /**
     ...
     * Uri of accounts / sip profiles
     */
    private static Uri accountUri = null;
     ...
     * @see ContentUris#appendId(android.net.Uri.Builder, long)
     */
    private static Uri baseAccountIdUri = null;

    /**
     * Uri for the sip profile adding/registration state.
     */
    private static Uri accountStatusUri = null;
     ...
     * @see ContentUris#appendId(android.net.Uri.Builder, long)
     */
    private static Uri baseAccountStatusIdUri = null;
     ...
     * 
     * @param context Your application context. Mainly useful to get the content provider for the request.
     ...
     */
    public static SipProfile getProfileFromDbId(Context context, long accountId, String[] projection) {
        SipProfile account = new SipProfile();
        if (accountId != INVALID_ID) {
            //20170224-mengbo : 动态获取URI
            //Cursor c = context.getContentResolver().query(ContentUris.withAppendedId(ACCOUNT_ID_URI_BASE, accountId),
            //      projection, null, null, null);
            Cursor c = context.getContentResolver().query(ContentUris.withAppendedId(getBaseAccountIdUri(context), accountId),
                    projection, null, null, null);
     ...
     */
    public static ArrayList<SipProfile> getAllProfiles(Context context, boolean onlyActive, String[] projection) {
        ArrayList<SipProfile> result = new ArrayList<>();
        ...
        Cursor c = context.getContentResolver().query(SipProfile.getAccountUri(context), projection, selection, selectionArgs, null);
    ...
    /**
     * Get uri of accounts / sip profiles
     */
    public static Uri getAccountUri(Context context) {
        if(accountUri == null || accountUri.equals(Uri.EMPTY)){
            if(context != null){
                accountUri = Uri.parse(ContentResolver.SCHEME_CONTENT + "://" + DBProvider.getAuthority(context) + "/" + ACCOUNTS_TABLE_NAME);
            }else{
                accountUri = Uri.EMPTY;
            }
        }
        return accountUri;
    }
    ...
    /**
     * Get base uri for the account / sip profile. <br/>
     */
    public static Uri getBaseAccountIdUri(Context context) {
        if(baseAccountIdUri == null || baseAccountIdUri.equals(Uri.EMPTY)){
            if(context != null){
                baseAccountIdUri = Uri.parse(ContentResolver.SCHEME_CONTENT + "://" + DBProvider.getAuthority(context) + "/" + ACCOUNTS_TABLE_NAME + "/");
            }else{
                baseAccountIdUri = Uri.EMPTY;
            }
        }
        return baseAccountIdUri;
    }

    /**
     * Get uri for the sip profile adding/registration state.
     */
    public static Uri getAccountStatusUri(Context context) {
        if(accountStatusUri == null || accountStatusUri.equals(Uri.EMPTY)){
            if(context != null){
                accountStatusUri = Uri.parse(ContentResolver.SCHEME_CONTENT + "://" + DBProvider.getAuthority(context) + "/" + ACCOUNTS_STATUS_TABLE_NAME);
            }else{
                accountStatusUri = Uri.EMPTY;
            }
        }
        return accountStatusUri;
    }

    /**
     * Get base uri for the sip profile adding/registration state.
     */
    public static Uri getBaseAccountStatusIdUri(Context context) {
        if(baseAccountStatusIdUri == null || baseAccountStatusIdUri.equals(Uri.EMPTY)){
            if(context != null){
                baseAccountStatusIdUri = Uri.parse(ContentResolver.SCHEME_CONTENT + "://" + DBProvider.getAuthority(context) + "/" + ACCOUNTS_STATUS_TABLE_NAME + "/");
            }else{
                baseAccountStatusIdUri = Uri.EMPTY;
            }
        }
        return baseAccountStatusIdUri;
    }
