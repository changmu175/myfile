...
//	private Button recordButton;
			...
			if(action.equals(SipManager.ACTION_SIP_CALL_CHANGED)){
				if(sipService != null) {
					try {
						...
						e.printStackTrace();
						LogUtil.getUtils(THIS_FILE).e("Not able to retrieve calls");
					}
				}
			}
		}
	...
	private ServiceConnection sipConnection = new ServiceConnection(){
		...
		public void onServiceConnected(ComponentName arg0, IBinder arg1) {
			LogUtil.getUtils(THIS_FILE).d("SipService is connected");
    ...
    private void updateUIFromMedia() {
        ...
        if (sipService != null) {
            try {
				...
				e.printStackTrace();
				LogUtil.getUtils(THIS_FILE).e("Sip service not avail for request ");
            }
        }
	...
	public void onProgressChanged(SeekBar arg0, int value, boolean arg2) {
		LogUtil.getUtils(THIS_FILE).d("Progress has changed");
		if(sipService != null) {
			try {
				...
				e.printStackTrace();
				LogUtil.getUtils(THIS_FILE).e("Impossible to set mic/speaker level");
			}
            ...
            while (true) {
                if (sipService != null) {
                    try {
						...
						e.printStackTrace();
						LogUtil.getUtils(THIS_FILE).e("Problem with remote service");
                        break;
                    }
                }
                ...
                try {
					...
					e.printStackTrace();
					LogUtil.getUtils(THIS_FILE).e("Interupted monitor thread");
                }
                synchronized (this) {
                    if (finished) {
                        break;
