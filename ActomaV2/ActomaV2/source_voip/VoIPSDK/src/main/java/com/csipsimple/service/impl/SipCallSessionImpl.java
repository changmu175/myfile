...
public class SipCallSessionImpl extends SipCallSession {
    ...
    public void setLastStatusComment(String lastStatusComment) {
        ...
        if(lastStatusCode >= 400 && lastStatusCode < 800)
        	this.lastStatusComment = StatusCommentReplace.ReplaceStatusComment(lastStatusCode);
    }
    ...
    public void applyDisconnect() {
        ...
        callStart = 0;  // callstart时间必须初始化。否则通话时长会异常。Modify by xjq. 20150807
        mediaSecureInfo = "";
        canRecord = false;
        isRecording = false;
        ...
        // rec device
        notifyRecDevice = true;

        // xjq
        statisticsStarted = false;
        rxStatistics.setValue(-1.0f);
        rxStatistics.setCompany(TrafficCompany.Byte);

        txStatistics.setValue(-1.0f);
        txStatistics.setCompany(TrafficCompany.Byte);
    ...
    public SipCallSessionImpl getCopy() {
    	SipCallSessionImpl impl = new SipCallSessionImpl();
    	Parcel p = Parcel.obtain();
    	writeToParcel(p, 0);
    	p.setDataPosition(0);
    	impl.initFromParcel(p);
    	p.recycle();
    	return impl;
    }
}
