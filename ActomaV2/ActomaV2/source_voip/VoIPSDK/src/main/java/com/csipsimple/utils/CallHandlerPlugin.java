...
public class CallHandlerPlugin {
    ...
    public void fillWith(String packageName, Bundle resolvedInfos) {
        ...
        pendingIntent = resolvedInfos.getParcelable(EXTRA_REMOTE_INTENT_TOKEN);
        icon = resolvedInfos.getParcelable(Intent.EXTRA_SHORTCUT_ICON);
    ...
    public static Long getAccountIdForCallHandler(Context ctxt, String packageName) {
        ...
        try {
            ...
            LogUtil.getUtils(THIS_FILE).e("Can't retrieve call handler cache id - reset");
        }
        if (accountId == SipProfile.INVALID_ID) {
            ...
            //edt.commit();
            /**2017-2-28 -wangzhen -start modify.change commit by apply for lint warning **/
            edt.apply();
            /**2017-2-28 -wangzhen -end **/
        }
        return accountId;
    }
    ...
    public static Map<String, String> getAvailableCallHandlers(Context ctxt) {
        ...
        if (AVAILABLE_HANDLERS == null) {
            ...
            for (ResolveInfo resInfo : availables) {
                ...
                LogUtil.getUtils(THIS_FILE).d("Found call handler " + actInfos.packageName + " " + actInfos.name);
