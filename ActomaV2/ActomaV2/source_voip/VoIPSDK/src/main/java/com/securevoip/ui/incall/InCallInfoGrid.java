...
public class InCallInfoGrid extends FrameLayout {
    ...
    private final ArrayList<View> mItems = new ArrayList<>();
    ...
    public void removeViewAt(int position) {
        if(position < 0 || position >= mItems.size()) {
            LogUtil.getUtils(THIS_FILE).w("Trying to remove unknown view at " + position);
        }else {
    ...
    private final Runnable postLayout = new Runnable() {
        ...
        public void run() {
            populate();
//            LogUtil.e(THIS_FILE, "IncallInfoGrid的postLayout的populate()执行了");
        }
    };
    ...
    synchronized protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        populate();
        LogUtil.getUtils().e(THIS_FILE + "InCallInfoGrid的onLayout()的populate()执行了");
    ...
    synchronized void populate() {
        ...
        LogUtil.getUtils(THIS_FILE).d("Populate " + count + " children");
        ...
        if(count > 0) {
            ...
            LogUtil.getUtils(THIS_FILE).v("Render a grid of " + numColumns + " x " + numRows);
            ...
            if(cellHeight < minRowHeight) {
                LogUtil.getUtils(THIS_FILE).d("May render weird... min height not correct " + cellHeight);
            }
            if(cellHeight <= 0 || cellWidth <= 0) {
                LogUtil.getUtils(THIS_FILE).w("The call grid cannot render " + cellHeight + "x" + cellWidth
                        + " for " + width + "x" + height);
        ...
        for (curIndex = 0; curIndex < mAdapter.getCount(); curIndex++) {
            ...
            if(p == null) {
                ...
                if(p == this) {
                    ...
                    LogUtil.getUtils(THIS_FILE).w("Call card already attached to somebody else");
                }
            }
            ii.forceLayout();
        }
    ...
    private class CallDataObserver extends DataSetObserver {
        ...
        public void onChanged() {
            populate();
            LogUtil.getUtils().e(THIS_FILE + "InCallInfoGrid的onChanged()的populate()执行了");
        }
        @Override
        public void onInvalidated() {
            populate();
            LogUtil.getUtils().e(THIS_FILE + "InCallInfoGrid的onInvalidated()的populate()执行了");
        }
    }
