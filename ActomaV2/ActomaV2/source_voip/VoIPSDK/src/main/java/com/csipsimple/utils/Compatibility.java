...
public final class Compatibility {
    ...
    public static boolean shouldUseRoutingApi() {
        ...
        // Samsung GT-S5830
        if (android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("GT-S")) {
            return true;
        }
...
//            return false;
//        }
        /**2017-2-28 -wangzhen modify.Simplify the if way**/
    ...
    public static boolean shouldUseModeApi() {
        ...
        if (android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("GT-") ||
                android.os.Build.PRODUCT.toUpperCase(Locale.getDefault()).startsWith("GT-") ||
                android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("YP-")) {
            return true;
        }
        ...
        // LG P500, Optimus V
        if (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("thunder")) {
            return true;
        }
        // LG-E720(b)
        if (android.os.Build.MODEL.toUpperCase(Locale.getDefault()).startsWith("LG-E720")
                && !Compatibility.isCompatible(9)) {
            return true;
        }
        // LG-G2
        if (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("g2")
                && android.os.Build.BRAND.toLowerCase().startsWith("lge")) {
            return true;
        }
        // LG-LS840
        if (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("cayman")) {
            return true;
        }
        ...
        if (android.os.Build.DEVICE.equalsIgnoreCase("U8150") ||
                ...
                android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("U8836") ||
        ...
        // Alcatel
        if (android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("ONE_TOUCH_993D")) {
            return true;
        }

        // N4
        if (android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("MAKO")) {
            return true;
        }
    ...
    public static String guessInCallMode() {
        ...
        if (!isCompatible(11) && android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("GT-I9100")) {
    ...
    public static String getDefaultFrequency() {
        ...
        if (android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).equals("GT-P1010")) {
    ...
    public static String getCpuAbi() {
        if (isCompatible(4)) {
            ...
            try {
                ...
                LogUtil.getUtils(THIS_FILE).e("Announce to be android 1.6 but no CPU ABI field", e);
            }
...
//                return false;
    ...
    private static boolean needPspWorkaround() {
        ...
        // All htc except....
        if (android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).startsWith("htc")
                || android.os.Build.BRAND.toLowerCase(Locale.getDefault()).startsWith("htc")
                || android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).equalsIgnoreCase("inc") /*
                                                                                   * For
                                                                                   * Incredible
                                                                                   */
...
//                return true;
                /**2017-2-28 -wangzhen modify.Change the if way Simplifly**/
                return !android.os.Build.DEVICE.equalsIgnoreCase("passion");
            }

        }
        // Dell streak
        if (android.os.Build.BRAND.toLowerCase(Locale.getDefault()).startsWith("dell") &&
                android.os.Build.DEVICE.equalsIgnoreCase("streak")) {
            return true;
        }
        // Motorola milestone 1 and 2 & motorola droid & defy not under 2.3
        if ((android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).contains("milestone2") ||
                android.os.Build.BOARD.toLowerCase(Locale.getDefault()).contains("sholes") ||
                android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).contains("sholes") ||
                android.os.Build.DEVICE.equalsIgnoreCase("olympus") ||
                android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).contains("umts_jordan")) && !isCompatible(9)) {
            return true;
        }
        // Moto defy mini
        if (android.os.Build.MODEL.equalsIgnoreCase("XT320")) {
...
//        if (android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).startsWith("gt-i5800") ||
//                android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).startsWith("gt-i5801") ||
//                android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).startsWith("gt-i9003")) {
//            return true;
//        }
//        return false;
        /**2017-2-28 -wangzhen modify.Change the if way Simplifly**/
        return (android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).startsWith("gt-i5800") ||
                android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).startsWith("gt-i5801") ||
                android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).startsWith("gt-i9003"));
    }
    ...
    private static boolean needSGSWorkaround() {
        ...
        if (android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("GT-I9000") ||
                android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("GT-P1000")) {
            return true;
        }
        return false;
    }
    ...
    private static boolean needWebRTCImplementation() {
        if (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).contains("droid2")) {
            return true;
        }
        if (android.os.Build.MODEL.toLowerCase(Locale.getDefault()).contains("droid bionic")) {
            return true;
        }
        if (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).contains("sunfire")) {
            return true;
        }
        // Huawei Y300
...
//        if (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("gt-")
//                || android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).startsWith("gt-")) {
//            return true;
//        }
//        return false;
        /**2017-2-28 -wangzhen modify.Change the if way Simplifly**/
        return (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("gt-")
                || android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).startsWith("gt-"));
    }

    private static boolean shouldFocusAudio() {
        /* HTC One X */
        if (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("endeavoru") ||
                android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("evita")) {
            return false;
        }

        if (android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("GT-P7510") && isCompatible(15)) {
            return false;
        }

        // mengbo 2016-08-13 start ace 5.0.2 系统 can't unFocus
        if (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("ace") && !isCompatible(22)) {
            return false;
        }
        // mengbo 2016-08-13 end

        // mengbo 2016-09-17 start hw-v1 5.0.2 系统 can't unFocus
        if (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("hw-v1") && isCompatible(21)) {
            return false;
        }
        // mengbo 2016-09-17 end
    ...
    private static int getDefaultAudioImplementation() {
        // Acer A510
//        if (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("picasso")) {
...
//        }
    ...
    private static void resetCodecsSettings(PreferencesWrapper preferencesWrapper) {
        ...
        preferencesWrapper.setCodecPriority("iLBC/8000/1", SipConfigManager.CODEC_NB, "235");
        ...
        preferencesWrapper.setCodecPriority("iLBC/8000/1", SipConfigManager.CODEC_WB, "235");
        ...
        preferencesWrapper.setPreferenceStringValue("band_for_3g", SipConfigManager.CODEC_WB); // 3g/4g 使用宽带配置 xjq
    ...
    public static void setFirstRunParameters(PreferencesWrapper preferencesWrapper) {
        ...
        /* BEGIN: Modified by xjq, 2015/1/14 PN:ecns,ec tail length, A6:100 P8:150 Other:150 */
        if(android.os.Build.MODEL.equalsIgnoreCase("ACTOM A6")) {
        	preferencesWrapper.setPreferenceStringValue(SipConfigManager.ECHO_CANCELLATION_TAIL, "100");
        }
        
        if(android.os.Build.MODEL.equalsIgnoreCase("P8")) {
        	preferencesWrapper.setPreferenceStringValue(SipConfigManager.ECHO_CANCELLATION_TAIL, "150");
        }
        /* END: Modified by xjq, 2015/1/14 */
        ...
        if (android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("GT-I9000") && !isCompatible(9)) {
...
//        return true;
        /**2017-2-28 -wangzhen modify.Change the if way Simplifly**/
        return (!android.os.Build.BRAND.equalsIgnoreCase("archos")
                && android.os.Build.DEVICE.equalsIgnoreCase("g7a"));
    }
    ...
    public static void updateVersion(PreferencesWrapper prefWrapper, int lastSeenVersion,
        ...
        if (lastSeenVersion < 911 && android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("GT-I9100")) {
            prefWrapper.setPreferenceStringValue(SipConfigManager.MICRO_SOURCE,
                    getDefaultMicroSource());
            prefWrapper
                    .setPreferenceStringValue(SipConfigManager.SIP_AUDIO_MODE, guessInCallMode());
        ...
        if (lastSeenVersion < 955 && android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).contains("droid2")) {
        ...
        if (lastSeenVersion < 1033 && android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).startsWith("thunder")) {
            prefWrapper
                    .setPreferenceBooleanValue(SipConfigManager.USE_MODE_API, shouldUseModeApi());
        }
        if (lastSeenVersion < 1076 && android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).equals("GT-P1010")) {
            prefWrapper.setPreferenceStringValue(SipConfigManager.SND_CLOCK_RATE,
                    getDefaultFrequency());
        }
        ...
        if (lastSeenVersion < 1634) {
            if (android.os.Build.PRODUCT.toLowerCase(Locale.getDefault()).startsWith("gt-i9003")) {
                prefWrapper.setPreferenceBooleanValue(SipConfigManager.SET_AUDIO_GENERATE_TONE,
                        needToneWorkaround());
            }
        }
        ...
        if (lastSeenVersion < 1798 && android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("picasso")) {
            prefWrapper.setPreferenceStringValue(SipConfigManager.AUDIO_IMPLEMENTATION,
                    Integer.toString(getDefaultAudioImplementation()));
        }
        ...
        if (lastSeenVersion < 2105 && android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("cayman")) {
            prefWrapper.setPreferenceBooleanValue(SipConfigManager.USE_MODE_API,
                    shouldUseModeApi());
        }
        ...
        if (lastSeenVersion < 2172 && android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("MAKO")) {
            prefWrapper.setPreferenceBooleanValue(SipConfigManager.USE_MODE_API,
                    shouldUseModeApi());
        }
        ...
        if (lastSeenVersion < 2302) {
            if (android.os.Build.DEVICE.toUpperCase(Locale.getDefault()).startsWith("U8836")) {
                prefWrapper.setPreferenceBooleanValue(SipConfigManager.USE_MODE_API,
                        shouldUseModeApi());
            }
        }
     ...
     * 是否需要录音权限必须开启
     */
    public static boolean shouldRecordAudioPermissionOn() {
        //诺亚信R6(诺亚信R6没开权限会引起崩溃)
        if (android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("ali6737m_35g_m0")) {
            return true;
        }
     ...
     * 是否需要重置媒体设备
     */
    public static boolean shouldResetSnd() {
        if(android.os.Build.DEVICE.toLowerCase(Locale.getDefault()).startsWith("candice")
                && isCompatible(23)){
            return true;
        }
