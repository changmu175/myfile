...
//import com.xdja.wx.frame.utils.StringUtils;

/**
 * modify by lixiaolong<mailto://lxl@xdja.com> on 20160907. 优化SharedPreferences的数据提交
 */
public class PreferencesWrapper {
    ...
    public static String sTunIP="123.15.49.77";
	...
	private static final long serialVersionUID = 1L;
	{
		...
		put(SipConfigManager.USE_SRTP, "0");//启动srtp Mod-lixin-0610
        ...
        put(SipConfigManager.TCP_KEEP_ALIVE_INTERVAL_WIFI, "240"); // 修改默认的TCP心跳时间为4分钟 xjq 2015-11-09
        put(SipConfigManager.TCP_KEEP_ALIVE_INTERVAL_MOBILE, "240");
        ...
        /**Begin:SUNYUNLEI 修改STUN 服务器默认地址**/
    	        
		put(SipConfigManager.STUN_SERVER, sTunIP);
//		put(SipConfigManager.STUN_SERVER, "192.168.12.126");
        
//        SysConfigDao scf=new SysConfigDao();
//        String stunIp=scf.getSysConfig(BaseDBHelper.TABLE_SYSCONFIG_C_STUN_IP);
//        if(stunIp == null || TextUtils.isEmpty(stunIp)){
//        	put(SipConfigManager.STUN_SERVER, "stun.counterpath.com");
//        }else{
//        	put(SipConfigManager.STUN_SERVER, stunIp);
//        }
        /**End:SUNYUNLEI 修改STUN 服务器默认地址**/
        ...
        // Set default sip server. Modify by xjq, 2015/5/13
//        put(SipConfigManager.SIP_SERVER_PORT, "120.194.4.154:5061");
	}};
	...
	{
		//Network
		put(SipConfigManager.LOCK_WIFI, true); // modify by xjq 2015-09-06
		put(SipConfigManager.LOCK_WIFI_PERFS, false);
		put(SipConfigManager.ENABLE_TCP,true);	//启动tls，关闭tcp
		put(SipConfigManager.ENABLE_UDP, true); //启动tls，关闭udp
		put(SipConfigManager.ENABLE_TLS, true);	//安全模块，启动tls-Mod-lixin-0609
		...
		/**Begin:SUNYUNLEI 修改STUN 服务器默认为开启**/
		//put(SipConfigManager.ENABLE_STUN, false);
        //put(SipConfigManager.ENABLE_STUN2, false);
		put(SipConfigManager.ENABLE_STUN, false);
        put(SipConfigManager.ENABLE_STUN2, false);
        /**End:SUNYUNLEI 修改STUN 服务器默认为开启**/
		put(SipConfigManager.ENABLE_QOS, false);
		put(SipConfigManager.USE_COMPACT_FORM, true);				//开启使用sip压缩信令-mod-lixin-0526
		...
		/**Begin:SUNYUNLEI 修改第一次使用时的简单配置中 允许使用移动网络**/
...
//		put(SipConfigManager.USE_EDGE_OUT, false);
		...
		/**End:SUNYUNLEI 修改第一次使用时的简单配置中 允许使用移动网络**/
		...
		//Media
		put(SipConfigManager.ECHO_CANCELLATION, true);							//回声抑制
        /* BEGIN: Modified by xjq, 2015/1/14 PN:ecns, enable vad */
		...
		put(SipConfigManager.AUTO_CONNECT_BLUETOOTH, true);// modify by xjq 2015-11-16 蓝牙耳机连接的情况下，加密电话自动使用蓝牙耳机
		...
		/**Begin:sunyunlei 去掉通知栏中显示 VOIP 图标*/
//		put(SipConfigManager.ICON_IN_STATUS_BAR, true);
		put(SipConfigManager.ICON_IN_STATUS_BAR, false);
		/**End:sunyunlei 去掉通知栏中显示 VOIP 图标*/
		put(SipConfigManager.USE_PARTIAL_WAKE_LOCK, false);
		put(SipConfigManager.ICON_IN_STATUS_BAR_NBR, false);
		//第一次使用时是否和 android 整合
		/**Begin:SUNYUNLEI 修改第一次使用程序时 是否整合android 的默认值为false**/
		...
		/**End:SUNYUNLEI 修改第一次使用程序时 是否整合android 的默认值为false**/
        ...
        /* BEGIN: Support multiple calls open.add by xjq, 2015/1/28 */
		put(SipConfigManager.SUPPORT_MULTIPLE_CALLS, true);
		/* END: Support multiple calls open.add by xjq, 2015/1/28 */
	...
	private final static HashMap<String, Float> FLOAT_PREFS = new HashMap<String, Float>(){
		private static final long serialVersionUID = 1L;
	{
		put(SipConfigManager.SND_MIC_LEVEL, (float)1.3);//1.0
		put(SipConfigManager.SND_SPEAKER_LEVEL, (float)1.3);//1.0
		put(SipConfigManager.SND_BT_MIC_LEVEL, (float)1.3);//1.0
		put(SipConfigManager.SND_BT_SPEAKER_LEVEL, (float)1.3);//1.0
		put(SipConfigManager.SND_STREAM_LEVEL, (float)10.0);//8.0
	}};
	...
	public void forceCheckUpgrade() {
        ...
        if (runningVersion != null) {
            ...
            /**Begin:Sunyunlei runningVersion 会随minifast.xml 中的VersonCode增长而增长，如果到一定数值Compatibility.updateVersion()方法中VOIP的参数可能不会设置，可能会导致一些不可预料的错误**/
            //editor.putInt(LAST_KNOWN_VERSION_PREF, runningVersion);
             editor.putInt(LAST_KNOWN_VERSION_PREF, 1);
            /**End:Sunyunlei runningVersion 会随minifast.xml 中的VersonCode增长而增长，如果到一定数值Compatibility.updateVersion()方法中VOIP的参数可能不会设置，可能会导致一些不可预料的错误**/
            editor.apply();
        }
    ...
    private Integer needUpgrade() {
        ...
        if (pinfo != null) {
			...
			LogUtil.getUtils(THIS_FILE).d("Last known version is " + lastSeenVersion
        ...
        if(prefs != null){
			...
			LogUtil.getUtils(THIS_FILE).d("Last known android version " + lastSeenVersion);
	...
	public Integer getPreferenceIntegerValue(String key) {
		try {
			...
			LogUtil.getUtils(THIS_FILE).d("Invalid "+key+" format : expect a int");
		}
	...
	public void resetAllDefaultValues() {
		...
		for(String key : FLOAT_PREFS.keySet() ) {
			editor.putFloat(key, FLOAT_PREFS.get(key));
		}
		editor.putBoolean(PreferencesProviderWrapper.HAS_ALREADY_SETUP_SERVICE, true);
		editor.apply();
	...
	public JSONObject serializeSipSettings() {
		...
		for(String key : STRING_PREFS.keySet() ) {
			try {
				...
				LogUtil.getUtils(THIS_FILE).e("Not able to add preference "+key);
			}
		}
		for(String key : BOOLEAN_PREFS.keySet() ) {
			try {
		...
		try {
			...
			LogUtil.getUtils(THIS_FILE).e("Not able to add last known version pref");
		}
	...
	public void restoreSipSettings(JSONObject jsonSipSettings) {
		...
		for(String key : STRING_PREFS.keySet() ) {
			try {
				...
				LogUtil.getUtils(THIS_FILE).e("Not able to get preference "+key);
			}
		}
		for(String key : BOOLEAN_PREFS.keySet() ) {
			try {
				setPreferenceBooleanValue(key, jsonSipSettings.getBoolean(key));
			} catch (JSONException e) {
				LogUtil.getUtils(THIS_FILE).e("Not able to get preference "+key);
			}
		}
	...
	public void addStunServer(String server) {
		if(!hasStunServer(server)) {
			...
			LogUtil.getUtils(THIS_FILE).d("Old stun > "+oldStuns+" vs "+STRING_PREFS.get(SipConfigManager.STUN_SERVER));
	...
	public short getCodecPriority(String codecName, String type, String defaultValue) {
		...
		if(key != null) {
		    try {
				...
				LogUtil.getUtils(THIS_FILE).e("Invalid codec priority", e);
		    }
		}
	...
	private static File getStorageFolder(Context ctxt, boolean preferCache) {
	    ...
	    if (root.canWrite()){
			...
			if(!dir.exists()) {
				dir.mkdirs();
				LogUtil.getUtils(THIS_FILE).d("Create directory " + dir.getAbsolutePath());
			}
			return dir;
	    }
	    return null;
