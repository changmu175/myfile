...
public class PreferencesProviderWrapper {
    ...
    private ConnectivityManager connectivityManager;  //主要管理和网络连接相关的操作
    ...
    public void resetAllDefaultValues() {
        ...
        //resolver.update(SipConfigManager.RAZ_URI, new ContentValues(), null, null);
        resolver.update(SipConfigManager.getRazURI(context), new ContentValues(), null, null);
    }
    ...
    private boolean isValidConnectionFor(NetworkInfo ni, String suffix) {
        if (isValidWifiConnectionFor(ni, suffix)) {
            LogUtil.getUtils(THIS_FILE).d("We are valid for WIFI");
            return true;
        }
        if (isValidMobileConnectionFor(ni, suffix)) {
            LogUtil.getUtils(THIS_FILE).d("We are valid for MOBILE");
            return true;
        }
        if (isValidOtherConnectionFor(ni, suffix)) {
            LogUtil.getUtils(THIS_FILE).d("We are valid for OTHER");
            return true;
        }
        if (isValidAnywayConnectionFor(ni, suffix)) {
            LogUtil.getUtils(THIS_FILE).d("We are valid ANYWAY");
            return true;
        }
        return false;
    }
    ...
    public ArrayList<String> getAllIncomingNetworks() {
        ArrayList<String> incomingNetworks = new ArrayList<>();
    ...
    public int getInCallMode() {
        ...
        try {
            ...
            LogUtil.getUtils(THIS_FILE).e("In call mode " + mode + " not well formated");
        }
    ...
    public long getClockRate() {
        ...
        try {
            ...
            LogUtil.getUtils(THIS_FILE).e("Clock rate " + clockRate + " not well formated");
        }
        return 16000;
    }
    ...
    public final static PackageInfo getCurrentPackageInfos(Context ctx) {
        ...
        try {
            ...
            LogUtil.getUtils(THIS_FILE).e("Impossible to find version of current package !!");
        }
        return pinfo;
    }
     ...
     */
    @SuppressLint("LoopStatementsThatDontLoop")
    ...
    public long getMediaQuality() {
        ...
        try {
            ...
            LogUtil.getUtils(THIS_FILE).e("Audio quality " + mediaQuality + " not well formated");
        }
        ...
        if (key != null) {
            ...
            if (!TextUtils.isEmpty(val)) {
                try {
                    ...
                    LogUtil.getUtils(THIS_FILE).e("Impossible to parse " + val);
                }
            }
        }
