...
public class UriUtils {
    ...
    public static String extractNumberFromIntent(Intent it, Context ctxt) {
        ...
        if(data != null && action != null) {
        	//获取到PhoneApp对象，获取Intent，并且取得拨出的号码然后，对号码进行处理
...
/*    *//**
    ...
    public static String subPhoneNumber(String urlNumber){
    	sunyunle<sip:12323213@111111>
  	  String[] str=urlNumber.split("@");
	  String num=str[0].substring(5);
	  return num;
    }*/
    /**
     * 截取电话号码组里面收费标识的电话号码 sunyunlei 20150202
     */
	/*public static String getSolePhoneNum(List<Mobile> listMobiles) {
		String phoneNum = "";
		
		if(listMobiles != null && listMobiles.size() > 0){
			
			for (int i = 0; i < listMobiles.size(); i++) {
				Mobile myMb = listMobiles.get(i);
				if (myMb.getType() == 1) {
					phoneNum = myMb.getMobile();
					break;
				}
			}
		}
