...
public class TimerWrapper extends BroadcastReceiver {
	...
	private static ExecutorService executorService = Executors.newFixedThreadPool(1); // 固定1个线程来执行任务
	...
	private synchronized void quit() {
		LogUtil.getUtils(THIS_FILE).v("Quit this wrapper");
		if(serviceRegistered) {
            serviceRegistered = false;
			try {
				service.unregisterReceiver(this);
			} catch (IllegalArgumentException e) {
				LogUtil.getUtils(THIS_FILE).e("Impossible to destroy timer wrapper", e);
			}
		}
...
//		hashOffset = hashOffset % 10;
	...
	*/
	...
	private synchronized int doSchedule(int entryId, int intervalMs) {
		...
		int existingReg = scheduleEntries.indexOf(entryId);
		if(existingReg != -1) {
		...
		// Push next
		LogUtil.getUtils(THIS_FILE).v("Schedule timer " + entryId + " in " + intervalMs + "ms @ " + firstTime);
        Compatibility.setExactAlarm(alarmManager, alarmType, firstTime, pendingIntent);
		scheduleEntries.add(entryId);
        scheduleTimes.add(firstTime);
		return 1;
	}
	
	private synchronized int doCancel(int entryId) {
		LogUtil.getUtils(THIS_FILE).v("Cancel timer " + entryId);
		alarmManager.cancel(getPendingIntentForTimer(entryId));
        int existingReg = scheduleEntries.indexOf(entryId);
        if(existingReg != -1) {
            scheduleEntries.remove(existingReg);
	...
	public void onReceive(Context context, Intent intent) {
		...
		if(TIMER_ACTION.equalsIgnoreCase(intent.getAction())) {
			if(singleton == null) {
				LogUtil.getUtils(THIS_FILE).w("Not found singleton");
				return;
			}
			int timerEntry = intent.getIntExtra(EXTRA_TIMER_ENTRY, -1);
			LogUtil.getUtils(THIS_FILE).v("FIRE Received TIMER " + timerEntry + " " +
	...
	public static int schedule(int entry, int entryId, int time) {
		if(singleton == null) {
			LogUtil.getUtils(THIS_FILE).e("Timer NOT initialized");
			return -1;
		}
    ...
    public static class SipTimersExecutor extends Handler {
        ...
        private void executeInternal(Runnable task) {
            try {
				/** 20170118-mengbo-start: 使用线程池，避免ANR **/
				executorService.submit(task);
				//task.run();
				/** 20170118-mengbo-end **/
            } catch (Throwable t) {
				LogUtil.getUtils(THIS_FILE).e("run task: " + task, t);
            }
        }
    }
	...
	private class TimerJob implements Runnable {
		...
		public void run() {
			...
			LogUtil.getUtils(THIS_FILE).v("FIRE START " + entryId);
			try {
			    ...
			    if(doFire) {
					...
					LogUtil.getUtils(THIS_FILE).w("Fire from old run " + entryId);
			    }
			}catch(Exception e) {
				LogUtil.getUtils(THIS_FILE).e("Native error ", e);
			}finally {
				wakeLock.release(this);
			}
			LogUtil.getUtils(THIS_FILE).v("FIRE DONE " + entryId);
